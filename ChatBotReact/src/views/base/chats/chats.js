...
const Chats = () => {
  ...
  useEffect(() => {
    ...
    const fetchProperties = async () => {
      try {
        const res = await axios.get("http://localhost:8080/properties/my", {
    ...
    const fetchUsers = async () => {
      try {
        const res = await axios.get(
          `http://localhost:8080/user/auth/property/${selectedProperty.id}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const fetchMessages = async () => {
    ...
    try {
      const [sentRes, receivedRes] = await Promise.all([
        axios.get(
          `http://localhost:8080/chat/${selectedProperty.id}/sent/${selectedChat.username}`,
          { headers: { Authorization: `Bearer ${token}` } }
        ),
        axios.get(
          `http://localhost:8080/chat/${selectedProperty.id}/${selectedChat.username}/received`,
          { headers: { Authorization: `Bearer ${token}` } }
        ),
      ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      await axios.post("http://localhost:8080/chat/send", payload, {
        headers: {
