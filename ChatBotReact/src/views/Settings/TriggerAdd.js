...
const TriggerAdd = ({ triggerTypes, departments, editTriggerId }) => {
  ...
  const fetchTrigger = async (editTriggerId) => {
    try {
      const response = await axios.get(`${API_URL}/chatbot/gettrigger/${editTriggerId}`)
  ...
  const handleSaveTrigger = async () => {
    try {
      ...
      let response
      if (editTriggerId) {
        response = await axios.patch(`${API_URL}/chatbot/UpdateTrigger/${editTriggerId}`, payload)
        alert('Trigger updated successfully!')
      } else {
        response = await axios.post(`${API_URL}/chatbot/AddTrigger`, payload)
              ...
              <div className="mx-auto rounded p-1" style={{
                ...
                <div className="input-group">
                  <input
                    ...
                    style={{ fontSize: '14px' }}
                  />
                  <CButton color="primary" size="sm" className="input-group-text ms-2">➤</CButton>
                </div>
              </div>
            </div>
          </CCol>
